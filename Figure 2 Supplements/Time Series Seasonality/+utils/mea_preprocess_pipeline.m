function struct_out = mea_preprocess_pipeline(os,...
...
neural_metrics.df = cat(2, neural_metrics.df, bursts.df,...
    network_bursts.df, spk_features.features, evoked_spk_features.df);

% Extract data for control and disease conditions
control = neural_metrics.df(control_wells, :, :);
disease = neural_metrics.df(disease_wells, :, :);

% Interpolate from first to final day in vitro
[control, ~] = utils.interpolate_data(control, div);
[disease, timepoints] = utils.interpolate_data(disease, div);

% Exclude wells that do not meet the minimum # of active electrodes
control = utils.active_elec_exclusion(...
    control, nactive_elecs, active_elecs_idx);
disease = utils.active_elec_exclusion(...
    disease, nactive_elecs, active_elecs_idx);

% Scale features
[struct_out.control_scaled, struct_out.disease_scaled,...
    struct_out.control_raw, struct_out.disease_raw] = ...
    utils.normalize_data(control, disease,...
    timepoints, div_first, div_final);

% Calculate normalized difference
[struct_out.control_norm_diff, struct_out.disease_norm_diff] = ...
    utils.calc_normalized_scores(...
    struct_out.control_scaled, struct_out.disease_scaled);

% Calculate the # of observations in control and disease conditions
control_sz = size(struct_out.control_norm_diff, 1);
disease_sz = size(struct_out.disease_norm_diff, 1);

% Define genotypes for each observation
struct_out.control_genotype = repmat(genotype, control_sz, 1);
struct_out.disease_genotype = repmat(genotype, disease_sz, 1);

% Define differentiation IDs for each observation
struct_out.control_diffid = repmat(diff_id, control_sz, 1);
struct_out.disease_diffid = repmat(diff_id, disease_sz, 1);

% Get matrix for # of active electrodes
struct_out.control_nae = control(:, active_elecs_idx, :);
struct_out.disease_nae = disease(:, active_elecs_idx, :);