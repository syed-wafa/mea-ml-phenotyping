function fdf = extract_spk_spikesorting(os, dirname, recording_times,...
    num_well_rows, num_well_cols, num_elec_rows, num_elec_cols)

% Function to extract electrode spike times from .spk files.
% Inputs:
%   os: operating system.
%   dirname: name of directory containing .spk files.
...
% Outputs:
%   fdf.tdf: electrode x well x time data frame of spike times.
%   fdf.vdf: electrode x well x time data frame of spike voltages.
...
tdf = num2cell(NaN(nelecs, nwells, nfiles));
vdf = num2cell(NaN(nelecs, nwells, nfiles));

for file = 1:nfiles % for each .spk file
    ...
    rec_start = recording_times(file, 1); rec_end = recording_times(file, 2);
    spike_data = file_data.SpikeData.LoadData([rec_start, rec_end]);
    ...
    % Format spiketimes into electrode x well x time data frame.
    w = 0;
            ...
            for elec_col = 1:num_elec_cols % for each electrode column
                for elec_row = 1:num_elec_rows % for each electrode row
                    ...
                    if ~isempty(template_match)
                        ...
                        t = horzcat(spike_data(template_match).GetTimeVector); t = t(1, :);
                        v = horzcat(spike_data(template_match).GetVoltageVector);
